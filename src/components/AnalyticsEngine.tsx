  ...
  return (
    <div className="bg-white/50 dark:bg-black/20 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-xl shadow-sm text-neutral-800 dark:text-neutral-200 h-full flex flex-col transition-all overflow-hidden relative group">
      ...
      <div className="p-[clamp(1rem,1.5vw,1.5rem)] flex-1 overflow-auto custom-scrollbar z-10 relative">
        {error && (
          <div className="p-3 mb-4 bg-white/30 dark:bg-black/30 backdrop-blur-3xl border border-black/5 dark:border-white/10 text-xs font-mono text-rose-400 rounded break-words">
            {'> Error:'} {error}
          </div>
        )}

        {!result && !analyzing && !error && (
          <div className="text-center py-8 text-[clamp(0.75rem,1.2vw,0.875rem)] font-mono text-neutral-500 dark:text-neutral-500 border border-dashed border-black/5 dark:border-white/10 rounded-lg bg-white/30 dark:bg-black/30 backdrop-blur-3xl/50 flex items-center justify-center min-h-[120px]">
        ...
        {result && (
          <div className="space-y-[clamp(1rem,1.5vw,1.5rem)]">
             <div className="flex items-center gap-3 bg-white/30 dark:bg-black/30 backdrop-blur-3xl/50 p-3 rounded-lg border border-black/5 dark:border-white/10">
               <span className="text-[clamp(0.65rem,0.8vw,0.7rem)] font-mono text-neutral-500 dark:text-neutral-500 uppercase tracking-widest">System Severity:</span>
             ...
             <div className="grid grid-cols-1 @md:grid-cols-2 gap-[clamp(1rem,1.5vw,1.5rem)] container-type-inline-size">
               <div className="bg-white/30 dark:bg-black/30 backdrop-blur-3xl border border-black/5 dark:border-white/10 p-4 rounded-xl space-y-4 hover:border-neutral-300 dark:border-neutral-700 transition-colors shadow-sm">
                 <h4 className="text-[clamp(0.7rem,1vw,0.75rem)] font-mono text-rose-400 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-2 uppercase tracking-widest">
                   <AlertCircle className="w-3.5 h-3.5" /> Detected Inefficiencies
                 </h4>
                 {result.issues.length > 0 ? (
                    <ul className="space-y-3">
                      {result.issues.map((issue, i) => (
                        <li key={i} className="text-[13px] font-sans text-neutral-700 dark:text-neutral-300 pl-3 border-l-2 border-rose-500/50 py-1 leading-relaxed">
                          {issue}
                        </li>
                      ))}
                    </ul>
                 ) : (
                   <div className="text-[13px] font-mono text-neutral-500 dark:text-neutral-500 py-2">No major issues detected.</div>
                 )}
               </div>

               <div className="bg-white/30 dark:bg-black/30 backdrop-blur-3xl border border-black/5 dark:border-white/10 p-4 rounded-xl space-y-4 hover:border-neutral-300 dark:border-neutral-700 transition-colors shadow-sm">
                 <h4 className="text-[clamp(0.7rem,1vw,0.75rem)] font-mono text-emerald-400 flex items-center gap-2 border-b border-black/5 dark:border-white/10 pb-2 uppercase tracking-widest">
                   <Lightbulb className="w-3.5 h-3.5" /> Opportunities
                 </h4>
                 {result.opportunities.length > 0 ? (
                    <ul className="space-y-3">
                      {result.opportunities.map((opp, i) => (
                        <li key={i} className="text-[13px] font-sans text-neutral-700 dark:text-neutral-300 pl-3 border-l-2 border-emerald-500/50 py-1 leading-relaxed">
                          {opp}
                        </li>
                      ))}
                    </ul>
                 ) : (
                   <div className="text-[13px] font-mono text-neutral-500 dark:text-neutral-500 py-2">No apparent opportunities.</div>
                 )}
               </div>
             </div>
          </div>
