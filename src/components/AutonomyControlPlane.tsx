...
export function AutonomyControlPlane() {
   ...
   return (
    <div className="bg-white/50 dark:bg-black/20 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-xl shadow-sm text-neutral-800 dark:text-neutral-200 h-full flex flex-col transition-all overflow-hidden relative group">
      ...
      <div className="p-[clamp(1rem,1.5vw,1.5rem)] border-b border-black/5 dark:border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-black/5 dark:bg-white/5 backdrop-blur-md z-10 relative">
        <div>
      ...
      <div className="p-[clamp(1rem,1.5vw,1.5rem)] space-y-[clamp(1rem,1.5vw,1.5rem)] flex-1 overflow-auto custom-scrollbar z-10 relative">
         ...
         <div className="grid grid-cols-1 @sm:grid-cols-2 gap-4 container-type-inline-size">
           <div className="bg-white/30 dark:bg-black/30 backdrop-blur-3xl/50 p-3 rounded-lg border border-black/5 dark:border-white/10">
             ...
             <select 
               ...
               className="w-full bg-white/50 dark:bg-black/20 backdrop-blur-xl border border-black/5 dark:border-white/10 text-xs p-2 rounded-md focus:ring-1 focus:ring-orange-500 outline-none text-neutral-800 dark:text-neutral-200 transition-colors hover:border-neutral-300 dark:border-neutral-700"
             >
         ...
         <div className="bg-white/30 dark:bg-black/30 backdrop-blur-3xl border border-black/5 dark:border-white/10 rounded-xl p-[clamp(1rem,1.5vw,1.25rem)] shadow-sm hover:border-neutral-300 dark:border-neutral-700 transition-colors">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-black/5 dark:border-white/10 pb-4 mb-4 gap-4">
               <div>
                  <h3 className="text-[clamp(0.75rem,1vw,0.875rem)] font-sans font-medium text-neutral-900 dark:text-white flex items-center gap-2">
                     Optimization Daemon
                  </h3>
                  <p className="text-[clamp(0.65rem,0.8vw,0.7rem)] font-mono text-neutral-500 dark:text-neutral-500 mt-1">
                     Continuously monitors metrics and queues workflows to Governance Layer.
                  </p>
               </div>
               <div className="text-left sm:text-right">
                  <span className="text-[clamp(0.6rem,0.7vw,0.65rem)] font-mono text-neutral-500 dark:text-neutral-500 uppercase tracking-widest block">Last Cycle</span>
                  <span className="text-xs font-mono text-neutral-700 dark:text-neutral-300">{lastRun || 'Never'}</span>
               </div>
             </div>
             ...
             {running ? (
               <div className="flex flex-col items-center justify-center py-8 gap-4 min-h-[150px]">
                 ...
                 <span className="text-[clamp(0.65rem,0.8vw,0.7rem)] font-mono text-neutral-600 dark:text-neutral-400 uppercase tracking-widest text-center">Executing Optimization Cycle...</span>
               </div>
             ) : result ? (
                <div className="space-y-4 min-h-[150px] flex flex-col justify-center">
                  {result.error ? (
                    <div className="p-4 bg-white/50 dark:bg-[#1e1e1e]/50 backdrop-blur-3xl border border-rose-500/20 text-[13px] font-mono text-rose-400 rounded-lg shadow-inner">
                      {result.error}
                    </div>
                  ) : (
                    <div>
                        ...
                        <div className="p-4 bg-white/50 dark:bg-black/20 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-lg text-[13px] font-mono space-y-2 shadow-inner">
                           <p><span className="text-neutral-500 dark:text-neutral-500">Action:</span> <span className="text-neutral-800 dark:text-neutral-200">{result.recommendation?.title}</span></p>
                           <p><span className="text-neutral-500 dark:text-neutral-500">Risk:</span> <span className={`${result.recommendation?.riskLevel === 'High' ? 'text-rose-400' : 'text-yellow-400'}`}>{result.recommendation?.riskLevel}</span></p>
                           <p className="text-neutral-600 dark:text-neutral-400 mt-3 whitespace-pre-wrap leading-relaxed border-t border-black/5 dark:border-white/10 pt-2">{result.recommendation?.description}</p>
                        </div>
                    </div>
                  )}
                </div>
               ...
               <div className="py-12 text-center flex items-center justify-center min-h-[150px]">
                 <p className="text-[clamp(0.75rem,1.2vw,0.875rem)] font-sans text-neutral-500 dark:text-neutral-500">System is idle. Waiting for next scheduled interval or manual trigger.</p>
               </div>
             )}
         </div>
      </div>
